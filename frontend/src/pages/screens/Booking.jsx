  ...
  return (
    <>
      <div className='outer' style={maintb}>
        ...
        <div style={{display:'flex',justifyContent:'end', marginBottom:'2rem'}} className="search">
          <input
            type="text"
            placeholder="Search..."
        ...
        <div>
          {list.map((booking, index) => (
            <div className="list-booking" key={index} style={tableBody}>
              ...
              <h5 className="name">{new Date(booking.selectedDate).toISOString().split('T')[0]}</h5>
              <h5 className="time">{booking.selectedTime}</h5>
              <h5 className="game">{booking.selectedGame}</h5>
              <h5 className="game">{booking.price}</h5>
              
              <h5 className="status" style={{ color: booking.isBooked ? 'green' : booking.isRejected || booking.isCancel ? 'red' : 'orange' }}>{booking.isBooked ? 'Success' : booking.isRejected || booking.isCancel ? 'Cancel' : 'Pending'}</h5>
              <h5>{booking.isCancel || booking.isRejected ? null :<Button onClick={() => handleCancel(booking._id,index)} >Cancel</Button>}</h5>
       ...
       <div className="pagination-search-controls">
        <div className="pagination">
          ...
          <span>Page: {page}</span>
          <button onClick={() => handlePageChange(page + 1)} disabled={list.length < limit}>
            Next
          </button>
        </div>
      </div>
