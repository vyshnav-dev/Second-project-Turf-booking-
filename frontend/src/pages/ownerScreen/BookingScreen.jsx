...
function BookingScreen() {
 ...
 const filteredList = list && list.length > 0
  ? list.filter((item) => {
      if (item.username && item.selectedGame) {
        return (
          item.username.toLowerCase().includes(searchQuery.toLowerCase()) ||
          (item.phoneNumber && item.phoneNumber.includes(searchQuery)) ||
          item.selectedGame.toLowerCase().includes(searchQuery.toLowerCase())
        );
      } else {
        return false;
      }
  ...
  return (
    ...
    <div style={{ display: 'flex', justifyContent: 'end', marginBottom: '1rem', paddingRight:'8rem' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <Table style={{ marginTop: '3rem', width: '80rem' }}striped bordered hover responsive variant='light'>
        <thead>
          <tr>
            <th>No</th>
            <th>TURF NAME</th>
        ...
        <tbody>
          {filteredList.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{new Date(item.selectedDate).toLocaleDateString('en-GB')}</td>
              <td>{item.selectedTime}</td>
              <td>{item.selectedGame}</td>
              <td style={{ color: item.isBooked ? 'green' : item.isRejected || item.isCancel ? 'red' : 'orange' }}>{item.isBooked ? 'Success' : item.isRejected || item.isCancel ? 'Cancel' : 'Pending'}</td>
              <td>
